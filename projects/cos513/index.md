...
$$F = AL$$
where \\(F\\) is the \\(k \times 92\\) matrix of frequency vectors for the given channel for each of the \\(92\\) pairs; L is the \\(m \times 92\\) matrix of image vectors for each of the \\(92\\) pairs, and \\(A\\) is \\(k x m\\) matrix that we learn for each of the \\(306\\) channels. 
...
Here we plot the \\(14^{th}\\) channel MEG response of the first individual, first trial for the first image:
