...
If we take \\(X\\) to be EEG-space and \\(Y\\) to be fMRI space, we essentially wish to find two functions \\(f_1(X, Y) = X'\\) and \\(f_2(X, Y) = Y'\\) such that \\(X'\\) and \\(Y'\\) are better representations of the underlying information in the brain data. 
